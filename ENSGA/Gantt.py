...
def gantt(figure_num, schedule_stime, plt_item):
    ...
    for single_scheduling in plt_item:
        ...
        plt.text(x=start_time + ((end_time - start_time) / 2 - 12), y=car_id - 0.05, s=destination, size=10)
...
def line_chart(fitness_label, fitness_iter_list, iteration_num):
    iteration_num_list = [_ for _ in range(0, iteration_num)]
    plt.rcParams['font.sans-serif'] = ['SimSun']
    plt.rcParams['font.size'] = 35
    plt.plot(iteration_num_list, fitness_iter_list, label='fitness '+str(fitness_label+1), linewidth=4, zorder=1)
    # x = [i for i in range(iteration_num)]
    # y = [j for j in fitness_iter_list]
    # plt.scatter(x, y, color='yellow', marker='p', s=50, zorder=2)
    plt.xlabel("iteration number", fontproperties='SimSun', size=35)
    plt.ylabel("fitness value", fontproperties='SimSun', size=35)
